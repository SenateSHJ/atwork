/**
 * render-atwork-linkedin.ts. Renders the LinkedIn section of the atWork
 * monthly report via the full production code path: silver reads through
 * the atWork LinkedIn shim, DB-loaded config, ALL_RULES arbitration.
 * Prints the raw SectionReport verbatim.
 *
 * Used to verify the LinkedIn integration end-to-end for the first time
 * (2026-08-31 session). Follows the render-atwork-meta.ts pattern.
 *
 * Run with:
 *   node --env-file=.env.local --import tsx scripts/render-atwork-linkedin.ts [--period=YYYY-MM]
 */
...
import { fetchAtWorkLinkedinPeriod } from '../src/app/monthly-reports/adapters/linkedin';
...
async function main(): Promise<void> {
  ...
  if (!url || !key) {
    console.error('render-atwork-linkedin: NEXT_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY required.');
    process.exit(1);
  }
  const sb = createClient(url, key, { auth: { persistSession: false } });

  console.log(`\n=============================================================`);
  console.log(`PRISM engine  → running section: LinkedIn`);
  ...
  const [current, prev, history] = await Promise.all([
    fetchAtWorkLinkedinPeriod(period),
    fetchAtWorkLinkedinPeriod(prior),
    buildHistory(fetchAtWorkLinkedinPeriod, period),
  ]);

  if (!current) {
    console.log('No LinkedIn data for this period. Try --period=YYYY-MM further back.');
    return;
  }
...
main().catch(err => {
  console.error('render-atwork-linkedin: failed:', err);
  process.exit(1);
});
