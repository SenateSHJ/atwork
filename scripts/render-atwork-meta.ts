/**
 * render-atwork-meta.ts. Renders the Meta section of the atWork
 * monthly report via the full production code path: silver adapter,
 * DB-loaded config, ALL_RULES arbitration. Prints the raw
 * SectionReport verbatim.
 *
 * Used to verify step 4 of the clone sequence: pipeline sound end-to-end
 * on one channel, no presentation layer.
 *
 * Run with:
 *   node --env-file=.env.local --import tsx scripts/render-atwork-meta.ts [--period=YYYY-MM]
 */
...
import { compose, loadConfig, DERIVED_RULES,
  ...
  flagDataCliff, describeAnchorDeltas, describeAnchorWeb, describeAnchorDeltasWeb,
...
async function main(): Promise<void> {
  ...
  if (!url || !key) {
    console.error('render-atwork-meta: NEXT_PUBLIC_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY required.');
    process.exit(1);
  }
  const sb = createClient(url, key, { auth: { persistSession: false } });

  console.log(`\n=============================================================`);
  console.log(`PRISM engine  → running section: Meta Ads`);
  ...
  const [current, prev, history] = await Promise.all([
    ...
    buildHistory(fetchAtWorkMetaPeriod, period),
  ]);

  if (!current) {
    console.log('No Meta data for this period. Try --period=YYYY-MM further back.');
    return;
  }
...
main().catch(err => {
  console.error('render-atwork-meta: failed:', err);
  process.exit(1);
});
